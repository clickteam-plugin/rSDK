...
typedef struct CValue {
	...
	union
	{
		long m_long;
		double m_double;
		LPSTR m_pString; // Never used in this version
	};
#ifdef __cplusplus
};
#else
...
#define	OCFLAGS2_OBSTACLE_SOLID		0x0000
#define	OCFLAGS2_OBSTACLE_PLATFORM	0x0010
#define	OCFLAGS2_OBSTACLE_LADDER	0x0020
...
#define		EVFLAGS_STEPAGAIN		0x08
#define		EVFLAGS_NOTDONEINSTART	0x10
#define		EVFLAGS_ALWAYS			0x20
#define		EVFLAGS_BAD				0x40
#define		EVFLAGS_BADOBJECT		0x80
#define		EVFLAGS_DEFAULTMASK		(EVFLAGS_ALWAYS+EVFLAGS_REPEAT+EVFLAGS_DEFAULT+EVFLAGS_STEPAGAIN+EVFLAGS_NOTDONEINSTART)
...
typedef struct tagRH4 {
	...
	char		rh4QuitString[128];						// FREE!!!! GREAT!
	...
	WORD		rh4FakeKey;					// For step through : fake key pressed
	...
	DWORD		rh4StringWork;				// String buffer position
...
typedef struct RunHeader {
	...
	BYTE		rhStopFlag;					// Current movement needs to be stopped
	BYTE		rhEvFlag; 					// Event evaluation flag
	...
	DWORD		rhReturn;					// Value to return to the editor
	DWORD		rhQuitParam;
...
// Buffers
	...
	LPDWORD		rhTimerEvents; 				// Timer pointers
	...
	int			rhMB_Count;					// Ball movement count
...
typedef struct headerObject {
	...
	OINUM  	hoOiPrev;					// Same OI previous object
	OINUM  	hoOiNext;					// ... next
...
typedef struct tagRM {
	...
	int		rmGoMovement;				// Branch to a new movement 
	...
	int  	rmEvOutLoops;				// If goes out of the frame
	int  	rmEvOutParam;
	BOOL	rmOutPlayfield;				// If goes out of the frame
	BOOL	rmColBack;					// Flag: Handle collisions with the background?
...
typedef	struct	tagRs {
	...
	union {
		...
		long		rsValue;			// Value counters
		float		rsValueFloat;		// Value counters
	};
	union
	{
		long			rsMini;
	...
	BYTE			rsHidden;
	BYTE			rsValueType;		// Type fo the value
...
#define	COMPARE_GT				5
	...
	#define		LOOPEXIT_SAVEGAME		6
	#define		LOOPEXIT_LOADGAME		7
