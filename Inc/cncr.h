...
typedef struct CRunFrame {
	...
	LPEVG			m_eventPrograms[MAX_EVENTPROGRAMS];
	LPOIL			m_oiList;
	LPVOID			m_free;
	...
	DWORD			m_eventProgramsSize[MAX_EVENTPROGRAMS];
	DWORD			m_programSize;
	LPBYTE			m_swapBuffers;
	DWORD			m_objectList;
	LPBYTE			m_destroyList;
	int				m_numOfEventPrograms;
	int				m_nPlayers;
	int				m_nPlayersReal;
...
typedef struct CRunApp {
	...
	BOOL			m_bMenuImagesLoaded;	// 1 if the images have been loaded into memory
	...
	LPDWORD			m_pGlobalValues;		// Global values
	LPBYTE			m_pGlobalTypes;			// Types
